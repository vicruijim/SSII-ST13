...
def ver_hashes(dir):
    ...
    if len(list_ficheros) < len(datos_bd):
         for dato in datos_bd:
              if dato[0] not in list_ficheros:
                     cambio = True
                     print(f"El archivo {dato[0]} ha sido borrado")
    ...
    for fichero in list_ficheros:
        ...
        if a == None:
            cambio = True
            print(f"El fichero {fichero} ha sido modificado")
...
def comprobacion_diaria():
    ...
    schedule.every(20).seconds.do(informe_mensual) #Se toman 60 segundos como un mes para probar que todo funcion 
    while True:
...
def informe_mensual():
     ...
     with open("./Logs/Logs.txt", "w") as archivo: #Se ha añadido aqui para realizar las pruebas 
            pass  
